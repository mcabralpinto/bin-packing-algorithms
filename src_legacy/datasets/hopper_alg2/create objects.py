...
for _ in range(rect_n - 1):
    ...
    while not valid:
        ...
        if side:
            point = random.randrange(current[0][0] + 1, current[1][0] - 1)
            new_objs = [[[current[0][0], current[0][1]], [point, current[1][1]]], [[point, current[0][1]], [current[1][0], current[1][1]]]]
        else:
            point = random.randrange(current[0][1] + 1, current[1][1] - 1)
            new_objs = [[[current[0][0], current[0][1]], [current[1][0], point]], [[current[0][0], point], [current[1][0], current[1][1]]]]
...
with open(fr"C:\Users\Utilizador\OneDrive - Universidade de Coimbra\UC\altri\bolsa_pcp\datasets\hopper_alg2\orders\my_problem{instance}.txt", "w") as problem:
