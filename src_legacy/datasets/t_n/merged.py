...
for i in range(problems):
    problem = open(fr"datasets\t_n\ng\blf_orders\t_n_problem{i + 5 * n}.txt", "w")
...
bl_file = open(fr"datasets\t_n\ng\objects{n}_bl.txt", "w")
bl_file.writelines(f"{problems}\n{new[1][0]} {new[1][1]}\n")


for i in range(problems):
    file = open(fr"datasets\t_n\ng\blf_orders\t_n_problem{i + 5 * n}.txt", "r")
    lines = file.readlines()
    file.close()
    ...
    for line in new_lines: bl_file.writelines(f"{line[0]} {line[1]} ")
    bl_file.writelines(f"\n")
